...
pushLeftAssoc (usageData name explicitOrNot whichArg) (exprLit (ident identifier₁)) with sameId name identifier₁
pushLeftAssoc (usageData name explicitOrNot whichArg) (exprLit (ident identifier₁)) | false = return $ exprLit (ident identifier₁) , nothing
pushLeftAssoc (usageData name explicitOrNot whichArg) (exprLit (ident identifier₁)) | true = return $ exprLit (ident identifier₁) , just (suc whichArg)
...
-- parameters can't be partially applied
-- TODO: This needs to be filled in properly when adding argument pushing for indexed types
pushParam : UsageData -> Parameter -> ScopeState Parameter
--pushParam ud (paramApp p p₁) = {!   !}
pushParam _ x = return x
...
pushUsagesSignature ud (typeSignature funcName funcType comments) = do
  newType <- pushType ud funcType
  return $ typeSignature funcName newType comments
...
pushUsages (usageData name t whichArg) (functionDefinition definitionOf params body range₁) | false = do
  newParams <- mapState (pushParam (usageData name t whichArg)) params
...
pushUsages (usageData name t whichArg) (functionDefinition definitionOf params body range₁) | true = do
  ...
  newBody <- pushExpr (usageData name t whichArg) body
  return $ functionDefinition definitionOf newParams newBody range₁
pushUsages ud (dataStructure dataName parameters indexInfo constructors range₁) = do
  ...
  return $ dataStructure dataName newParams newIndex newCons range₁
pushUsages _ code = return code

_doesNotAppearInExp_ : Identifier -> Expr -> ScopeState ⊤
x doesNotAppearInExp exprLit numLit = return tt
identifier name₁ isInRange₁ scope₁ declaration₁ doesNotAppearInExp exprLit (ident (identifier name isInRange scope declaration)) with compare declaration₁ declaration
(identifier name₁ isInRange₁ scope₁ declaration₁ doesNotAppearInExp exprLit (ident (identifier name isInRange scope .declaration₁))) | equal .declaration₁ = fail "Argument to be pushed is used in the type of the next argument"
... | _ = return tt
...
x doesNotAppearInArg typeSignature funcName funcType comments = x doesNotAppearIn funcType
...
namedArgument (typeSignature funcName funcType comments) isNotUsedIn y = funcName doesNotAppearIn y
x isNotUsedIn y = return tt

_hasDifferentNameFrom_ : Type -> Type -> ScopeState ⊤
namedArgument (typeSignature (identifier name isInRange scope declaration) funcType comments) hasDifferentNameFrom namedArgument (typeSignature (identifier name₁ isInRange₁ scope₁ declaration₁) funcType₁ comments₁) with name == name₁
...
pushArgument (signature (typeSignature (identifier name isInRange scope declaration) funcType comments) range₁ ∷ code) funcID whichArgument with compare declaration funcID
pushArgument (signature (typeSignature (identifier name isInRange scope declaration) funcType comments) range₁ ∷ code) .declaration whichArgument | equal .declaration = do
    newType <- pushArg funcType whichArgument
    newCode <- mapState ( pushUsages $ usageData (identifier name isInRange scope declaration) (isExplicit funcType) whichArgument) code
    return $ (signature (typeSignature (identifier name isInRange scope declaration) newType comments) range₁ ∷ newCode)
... | _ = do
  newxs <- pushArgument code funcID whichArgument
  return $ (signature (typeSignature (identifier name isInRange scope declaration) funcType comments) range₁) ∷ newxs
